    ...
    if (req.user) {
        ...
        try {
            if (req.query.id) { // GET USER RESERVATION BY ID
                ...
                result = await pool.query("SELECT room_id, num_of_adults, num_of_children, num_of_infants FROM reservations_rooms JOIN reservations ON reservations.id = reservations_rooms.reservation_id WHERE reservations_rooms.reservation_id = $1", [id]);
            ...
            } else { // GET ALL USER RESERVATIONS
                ...
                if (result.rows.length > 0) result.rows.forEach(({ id, checkin_date, checkout_date, total_price, created_at, status }) => {
