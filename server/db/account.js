...
export const updateUser = async(req, res) => {
    ...
    try { // Retrieve existing details from database if not provided in body
        let result = await pool.query("SELECT first_name, last_name, phone, email, password FROM guests WHERE id = $1", [userId]);
        ...
        let { firstName, lastName, phone, email, currentPassword, newPassword, confirmNewPassword } = req.body;
        ...
        if (currentPassword) {
            ...
            if (newPassword) {
                ...
                if (!checkPassword(newPassword)) return res.status(400).send("Error: New password must be 8 to 15 characters long and contain at least one uppercase letter, lowercase letter, numeric digit and special character.");
        ...
        // UPDATE USER DETAILS
        let text = "UPDATE guests SET first_name = $1, last_name = $2, phone = $3, email = $4, password = $5 WHERE id = $6 RETURNING id";
...
export const unlinkThirdParty = async (req, res) => {
    ...
    try { // Get third-party credentials from given provider
