...
export const login = async (req, accessToken, refreshToken, profile, done) => {
	...
	try {
		...
		// Get user details
		let text =
			"SELECT guests.id AS id, guests.email AS email FROM guests JOIN federated_credentials ON federated_credentials.guest_id = guests.id WHERE federated_credentials.id = $1 AND federated_credentials.provider = $2";
